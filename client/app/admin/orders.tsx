  ...
  const updateStatus = async (newStatus: string) => {
    ...
    try {
      ...
      const { data } = await api.put(
        `/order/${selectedOrder._id}/status`, 
              ...
              {updating ? (
                ...
                <FlatList
                  ...
                  renderItem={({ item }) => (
                    <TouchableOpacity
                      className={`
                        p-4 rounded-xl mb-2 flex-row justify-between items-center 
                        ${selectedOrder?.orderStatus === item ? "bg-primary/10" : "bg-gray-50"}
                      `}
                      onPress={() => updateStatus(item)}
                    >
                      <Text className={
                        `font-medium capitalize 
                        ${selectedOrder?.orderStatus === item ? "text-primary font-bold" : "text-secondary"}
                      `}>
                        {item}
                      </Text>
